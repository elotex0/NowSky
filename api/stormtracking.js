  ...
  const fetchXml = async () => {
    const results = await Promise.allSettled(
      [5, 10, 15, 20].map(async (offset) => {
        ...
        const url = `https://opendata.dwd.de/weather/radar/konrad3d/KONRAD3D_20260506T145500.xml`;
        const r = await fetch(url, {
          headers: { "User-Agent": "konrad3d-api/1.0" },
          signal: AbortSignal.timeout(12000),
        });
    ...
    if (nwp_mu_cape !== null) {
      ...
      if (nwp_mu_cape !== null && nwp_bs_06km !== null) {
        ...
        if (echo_top_msl !== null) {
          ...
          else if (echo_top_msl >= 8000)  hail_score += 10;
          else if (echo_top_msl >= 6000)  hail_score += 5;
