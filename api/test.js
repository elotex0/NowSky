...
function calcCIN(hour) {
    ...
    const t925 = hour.temp925 ?? ((t2m + (hour.temp850 ?? t2m)) / 2);
    const t850 = hour.temp850 ?? 0;
    const t700 = hour.temp700 ?? 0;

    if (t2m === 0 && t850 === 0) return 0;

    // ── ML-Parcel: 2m als Proxy für 50-hPa-Schichtmittel ──────────────
    // (open-meteo liefert kein echtes Schichtmittel; 2m ist vertretbar
    //  wenn kein starker SB-Superadiabat vorhanden ist)
    const dewDep = t2m - d2m;
    const T_LCL  = t2m - 0.212 * dewDep - 0.001 * dewDep * dewDep; // Bolton 1980
    const T_LCL_K = T_LCL + 273.15;
    const z_LCL   = Math.max(0, 125 * dewDep); // m über Boden

    // Theta-E des Parcels (Bolton 1980, pseudo-adiabatisch)
    const e_d   = 6.112 * Math.exp((17.67 * d2m) / (d2m + 243.5));
    const w_gkg = 1000 * 0.622 * e_d / (1013.25 - e_d);
    const T2m_K = t2m + 273.15;
    const theta_e = T2m_K
        * Math.pow(1000 / 1013.25, 0.2854 * (1 - 0.00028 * w_gkg))
        * Math.exp((3.376 / T_LCL_K - 0.00254) * w_gkg * (1 + 0.00081 * w_gkg));

    // ── Parcel-Temperatur iterativ für jedes Druckniveau ──────────────
    function parcelTempAt(pressHPa, startGuess) {
        let Tp = startGuess;
        for (let i = 0; i < 20; i++) {
            const Tp_K = Tp + 273.15;
            const es   = 6.112 * Math.exp((17.67 * Tp) / (Tp + 243.5));
            const ws   = Math.max(1e-6, 0.622 * es / (pressHPa - es));
            const ws_g = ws * 1000;
            const theta_e_test = Tp_K
                * Math.pow(1000 / pressHPa, 0.2854 * (1 - 0.00028 * ws_g))
    ...
    // ── Schichten: Oberfläche → LCL (trocken-adiabatisch) ─────────────
    // Unterhalb LCL: Parcel steigt trockenadiabatisch → DALR = 9.8 K/km
    // CIN kann nur unterhalb LFC auftreten; LFC liegt i.d.R. nahe LCL
    // wenn Atmosphäre nicht stark gedeckelt ist
    const DALR = 9.8; // K/km
    const g    = 9.81;

    // Niveau-Abstände (Standardatmosphäre, Druckkoordinaten):
    // ~1013 hPa = 0m, ~925 hPa = 750m, ~850 hPa = 1500m, ~700 hPa = 3000m
    const levels = [
        { p: 1013, z: 0,    Tenv: t2m  },
        { p: 950,  z: 540,  Tenv: hour.temp950 ?? (t2m + t850) / 2 - 1 },
        { p: 925,  z: 750,  Tenv: t925 },
        { p: 850,  z: 1500, Tenv: t850 },
        { p: 700,  z: 3000, Tenv: t700 },
    ];

    // Parcel-Temperatur an jedem Level
    // Unterhalb z_LCL: trockenadiabatisch; oberhalb: feuchtadiabatisch
    let cinSum = 0;
    let prevZ  = 0;
    let prevDT = 0; // Parcel - Umgebung am vorherigen Level
    ...
    for (let i = 1; i < levels.length; i++) {
        ...
        let Tparcel;
        if (z <= z_LCL) {
            // Trockenadiabatisch: Parcel kühlt mit DALR ab
            Tparcel = t2m - DALR * (z / 1000);
        } else if (z_LCL <= levels[i-1].z) {
            // Ganzes Intervall oberhalb LCL: feuchtadiabatisch
            const prevP = levels[i-1].p;
            const prevGuess = t2m - 6.5 * (z / 1000); // isothermale Näherung
            Tparcel = parcelTempAt(lev.p, prevGuess);
        } else {
            // LCL liegt innerhalb dieses Intervalls: Übergang
            // Startwert: feuchtadiabatisch von LCL aus
            const prevGuess = T_LCL - 6.0 * ((z - z_LCL) / 1000);
            Tparcel = parcelTempAt(lev.p, prevGuess);
        }

        const dT_top = Tparcel - Tenv;
        const dT_bot = prevDT;
        const dz     = z - prevZ;

        // Trapez-Integration: CIN nur wo Parcel kälter als Umgebung
        // Lineare Interpolation des Nulldurchgangs wenn Vorzeichen wechselt
        if (dT_bot <= 0 && dT_top <= 0) {
            // Ganzes Intervall negativ → volle Fläche
            const meanDT    = (dT_bot + dT_top) / 2;
            const T_mean_K  = (levels[i-1].Tenv + Tenv) / 2 + 273.15;
            cinSum += (meanDT / T_mean_K) * g * dz;
        } else if (dT_bot < 0 && dT_top > 0) {
            // Parcel wechselt zu positiv → LFC im Intervall, CIN endet hier
            const fracNeg   = -dT_bot / (dT_top - dT_bot); // Anteil negativ
            const dz_neg    = fracNeg * dz;
            const meanDT    = dT_bot / 2; // Trapez bis Nulldurchgang
            const T_mean_K  = levels[i-1].Tenv + 273.15;
            cinSum += (meanDT / T_mean_K) * g * dz_neg;
        } else if (dT_bot > 0 && dT_top < 0) {
            // Inversion beginnt erst im Intervall → nur unterer Teil positiv
            const fracPos   = dT_bot / (dT_bot - dT_top);
            const dz_neg    = (1 - fracPos) * dz;
            const meanDT    = dT_top / 2;
            const T_mean_K  = Tenv + 273.15;
            cinSum += (meanDT / T_mean_K) * g * dz_neg;
        }
        // dT_bot > 0 && dT_top > 0 → kein Beitrag zu CIN

        prevZ  = z;
        prevDT = dT_top;

        // Stoppe bei LFC (erster Nulldurchgang von unten nach oben)
        if (dT_top > 0 && prevDT <= 0) break;
        // Stoppe wenn Parcel schon lange positiv → EL erreicht
        if (dT_top > 2 && z > z_LCL) break;
    }

    return Math.round(Math.max(-500, Math.min(0, cinSum)));
}
